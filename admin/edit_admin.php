...
if ($_POST) {
  ...
  if ($login && $login['admin_id'] == $admin_id) {
    $statement = $pdo->prepare(
      "UPDATE admin SET name=:name, phone_number=:phone_number, email=:email WHERE id=:admin_id"
    );
    ...
    $statement = $pdo->prepare(
      "UPDATE login SET email=:email WHERE admin_id=:admin_id"
    );
    $statement->bindValue(":email", $_POST['email']);
    $statement->bindValue(":admin_id", $admin_id);
    $statement->execute();
...
$statement = $pdo->prepare("SELECT * FROM admin WHERE id=:admin_id");
$statement->bindValue("admin_id", $admin_id);
$statement->execute();
